...
export const PinAnalyzer = () => {
  ...
  const handleDownload = (pinData: PinData) => {
    // Create CSV content
    const csvContent = [['URL', 'Title', 'Description', 'Keywords', 'Pin Score', 'Saves', 'Clicks', 'Impressions', 'Engagement', 'Status'], [pinData.url, pinData.title, pinData.description, pinData.keywords.join(', '), pinData.stats.pinScore, pinData.stats.saves, pinData.stats.clicks, pinData.stats.impressions, `${pinData.stats.engagement}%`, pinData.status]].map(row => row.map(cell => `"${cell}"`).join(',')).join('\n');
  ...
  return <div className="w-full space-y-6">
      <form onSubmit={handleAnalyzePin} className="w-full mb-8">
        <div className="flex flex-col sm:flex-row gap-2">
          <div className="relative flex-1">
            <Input type="url" placeholder={t('enterPinUrl')} value={pinUrl} onChange={e => setPinUrl(e.target.value)} className="pr-10" required />
            <div className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
              <Search className="h-4 w-4 text-gray-400" />
            </div>
          </div>
          <Button type="submit" disabled={isLoading || !pinUrl} className="gap-2 text-white bg-pinterest-red">
            {isLoading ? <>
      ...
      {/* Results Table */}
      {pinResults.length > 0 ? <div className="rounded-md border bg-white dark:bg-gray-900">
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {pinResults.map(pin => <TableRow key={pin.id}>
                  ...
                  <TableCell className="max-w-[300px]">
                    <div className="flex flex-wrap gap-1">
                      {pin.keywords.map((keyword, i) => <span key={i} className="inline-flex items-center rounded-full bg-gray-100 dark:bg-gray-800 px-2 py-0.5 text-xs text-gray-800 dark:text-gray-200">
                          {keyword}
                        </span>)}
                    </div>
                  </TableCell>
                  <TableCell>
                    <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs ${pin.status === 'completed' ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' : 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200'}`}>
                      {pin.status}
                    </span>
                  </TableCell>
        ...
        </div> : <div className="text-center py-10 bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800">
