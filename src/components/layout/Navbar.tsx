      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="hidden md:flex items-center justify-center flex-1 space-x-6">
            ...
            <Link to="/pricing" className="px-4 py-2 rounded-md text-gray-700 dark:text-gray-300 hover:bg-black dark:hover:bg-black hover:text-pinterest-red dark:hover:text-pinterest-red hover:shadow-[0_0_12px_rgba(234,56,76,0.6)] hover:border hover:border-white/20 dark:hover:border-white/20 transition-all duration-300">
              {t('pricing')}
            </Link>
          </nav>
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            {user ? <Button variant="default" size="sm" className="gap-2 bg-pinterest-red border border-white/10 animate-glow-pulse" onClick={() => navigate('/dashboard')}>
                ...
                <Button variant="ghost" size="sm" className="gap-2 border border-white/10 dark:border-white/5 hover:shadow-[0_0_12px_rgba(234,56,76,0.6)]" onClick={() => navigate('/auth')}>
        ...
        {mobileMenuOpen && <div className="md:hidden absolute top-16 left-0 right-0 bg-white dark:bg-black shadow-md p-4 flex flex-col gap-4 border-t border-white/10 dark:border-white/5">
            ...
            <button onClick={() => scrollToSection('features')} className="text-left text-gray-600 dark:text-gray-300 hover:text-pinterest-red hover:glow-red transition-colors py-2">
              {t('features')}
            </button>
            
            <button onClick={() => scrollToSection('how-it-works')} className="text-left text-gray-600 dark:text-gray-300 hover:text-pinterest-red hover:glow-red transition-colors py-2">
              Try It
            </button>
            
            <Link to="/pricing" className="text-gray-600 dark:text-gray-300 hover:text-pinterest-red hover:glow-red transition-colors py-2" onClick={() => setMobileMenuOpen(false)}>
              {t('pricing')}
            </Link>
          ...
          setMobileMenuOpen(false);
                ...
                <Button variant="ghost" size="sm" className="justify-start gap-2 border border-white/10 dark:border-white/5 hover:shadow-[0_0_12px_rgba(234,56,76,0.6)]" onClick={() => {
            navigate('/auth');
            setMobileMenuOpen(false);
          }}>
                  <LogIn className="h-4 w-4" />
